...
| 行为 | 传入单个笔记内容长度 | `4000` 字符 | 每篇各自截断，保留开头部分。 |
...
无论选了哪一档，**选中超过一篇笔记时始终会先弹确认窗口**。批量改名是破坏性操作，而思源的重命名没有撤销栈。

### 关闭思考

推理模型可能很慢，有些还会把全部输出放进推理字段而让 `content` 留空。**是否禁用思考** 开关会在请求体里带上 `reasoning_effort: "none"`。
...
| `HTTP 400` 且提到未知参数 | 供应商不认 `reasoning_effort`。关掉 **是否禁用思考**。 |
